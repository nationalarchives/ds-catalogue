from app.lib.fields import (
    ...
    DateComponentField,  # Make sure this is imported
)
...
class CatalogueSearchForm(BaseForm):
    ...
    def cross_validate(self) -> list[str]:
        ...
        if doc_from and doc_to and doc_from > doc_to:
            errors.append("Document 'from' date must be before or equal to 'to' date")
        ...
        if opening_from and opening_to and opening_from > opening_to:
            errors.append("Opening 'from' date must be before or equal to 'to' date")
    ...
    def get_api_date_params(self) -> dict:
        ...
        # Record dates
        if rd_from := self.fields[FieldsConstant.RECORD_DATE_FROM].format_for_api():
        ...
        # Opening dates
        if od_from := self.fields[FieldsConstant.OPENING_DATE_FROM].format_for_api():
            params['opening_date_from'] = od_from
        if od_to := self.fields[FieldsConstant.OPENING_DATE_TO].format_for_api():
