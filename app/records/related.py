...
TNA_SIMILAR_LEVELS = {
    1: [1, 2],      # Department: show other Departments and Divisions
    2: [1, 2, 3],   # Division: show Departments, Divisions, and Series  
    3: [2, 3, 4],   # Series: show Divisions, Series, and Sub-series
    4: [3, 4, 5],   # Sub-series: show Series, Sub-series, and Sub-sub-series
    5: [4, 5, 6],   # Sub-sub-series: show Sub-series, Sub-sub-series, and Pieces
    6: [5, 6, 7],   # Piece: show Sub-sub-series, Pieces, and Items
    7: [6, 7],      # Item: show Pieces and Items
}
...
def get_related_records_by_subjects(
    ...
    if not similar_levels:
        logger.warning(f"No similar levels found for TNA level_code {current_record.level_code}")
        return []
    ...
    if len(current_record.subjects) <= 3:
        records = _search_with_all_subjects(current_record, similar_levels, limit)
    ...
    for level in similar_levels:
        ...
        try:
            ...
            for record in api_result.records:
                ...
                if record.iaid != current_record.iaid and record.iaid not in seen_iaids:
...
def _search_and_rank_by_subject_matches(
    ...
    record_matches = {}  # {iaid: (Record, set_of_matching_subjects, level_distance)}
            ...
            try:
                ...
                for record in api_result.records:
                    ...
                    if record.iaid not in record_matches:
                        ...
                        record_matches[record.iaid] = (record, set(), level_distance)
                    
                    # Add this subject to the matching subjects set
                    record_matches[record.iaid][1].add(subject)
                    
            except Exception as e:
                logger.debug(f"Failed to search for subject '{subject}' at level '{level}': {e}")
                continue
