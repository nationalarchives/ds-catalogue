...
class CatalogueSearchViewTests(TestCase):
    ...
    def test_catalogue_search_context_without_params(self):
        ...
        # Test subjects field
        self.assertEqual(
            self.response.context_data.get("form").fields["subject"].name,
            "subject",
        )
        self.assertEqual(
            self.response.context_data.get("form").fields["subject"].label,
            "Subjects",
        )
        self.assertEqual(
            self.response.context_data.get("form").fields["subject"].value, []
        )
        self.assertEqual(
            self.response.context_data.get("form").fields["subject"].cleaned,
            [],
        )
        self.assertEqual(
            self.response.context_data.get("form").fields["subject"].items,
            [
...
class CatalogueSearchViewDebugAPITnaBucketTests(TestCase):
    ...
    def test_catalogue_debug_api(self, mock_logger):
        ...
        self.assertEqual(self.response.status_code, HTTPStatus.OK)
        mock_logger.debug.assert_called_with(
            "https://rosetta.test/data/search?aggs=level&aggs=collection&aggs=closure&aggs=subjects&filter=group%3Atna&filter=coveringFromDate%3A%28%3E%3D2000-12-1%29&filter=coveringToDate%3A%28%3C%3D2000-12-31%29&q=%2A&size=20"
        )
        ...
        self.assertEqual(self.response.status_code, HTTPStatus.OK)
        mock_logger.debug.assert_called_with(
            "https://rosetta.test/data/search?aggs=level&aggs=collection&aggs=closure&aggs=subjects&filter=group%3Atna&filter=openingFromDate%3A%28%3E%3D2000-12-1%29&filter=openingToDate%3A%28%3C%3D2000-12-31%29&q=%2A&size=20"
        )
