...
Tests prove that:
1. Delivery Options API has a timeout (defined in constants)
2. Other APIs (Rosetta, Wagtail) have no timeout (None)
"""
...
from app.records.api import wagtail_request_handler, rosetta_request_handler
...
class TimeoutConfigurationTests(TestCase):
    ...
    @patch("django.conf.settings.DELIVERY_OPTIONS_API_URL", "https://api.test.com/delivery-options")
    def test_delivery_options_has_configured_timeout(self, mock_get):
        ...
        The timeout value is defined in app.deliveryoptions.constants.DELIVERY_OPTIONS_API_TIMEOUT
        """
        ...
        self.assertEqual(call_kwargs['timeout'], DELIVERY_OPTIONS_API_TIMEOUT,
                        f"Expected timeout={DELIVERY_OPTIONS_API_TIMEOUT}, got timeout={call_kwargs['timeout']}")

    def test_delivery_options_timeout_constant_is_defined(self):
        """
        Test that DELIVERY_OPTIONS_API_TIMEOUT constant is properly defined.
        
        This ensures the constant exists and has a reasonable value.
        """
        self.assertIsNotNone(DELIVERY_OPTIONS_API_TIMEOUT,
                           "DELIVERY_OPTIONS_API_TIMEOUT should not be None")
        self.assertIsInstance(DELIVERY_OPTIONS_API_TIMEOUT, int,
                            f"DELIVERY_OPTIONS_API_TIMEOUT should be an integer, got {type(DELIVERY_OPTIONS_API_TIMEOUT)}")
        self.assertGreater(DELIVERY_OPTIONS_API_TIMEOUT, 0,
                          f"DELIVERY_OPTIONS_API_TIMEOUT should be positive, got {DELIVERY_OPTIONS_API_TIMEOUT}")
        self.assertLessEqual(DELIVERY_OPTIONS_API_TIMEOUT, 60,
                           f"DELIVERY_OPTIONS_API_TIMEOUT should be reasonable (≤60s), got {DELIVERY_OPTIONS_API_TIMEOUT}")

    @patch("app.lib.api.get")
    @patch("django.conf.settings.ROSETTA_API_URL", "https://api.test.com/rosetta")
    def test_rosetta_api_has_no_timeout(self, mock_get):
        """
        CRITICAL TEST: Verify Rosetta API is called with NO timeout (timeout=None).
        
        This test proves that the Rosetta API (used for record fetching) 
        does NOT have a timeout, maintaining backward compatibility.
        """
        # Mock successful response
        mock_response = MagicMock()
        mock_response.status_code = 200
    ...
    @patch("app.lib.api.get")
    @patch("django.conf.settings.WAGTAIL_API_URL", "https://api.test.com/wagtail")
    def test_wagtail_api_has_no_timeout(self, mock_get):
        """
        CRITICAL TEST: Verify Wagtail API is called with NO timeout (timeout=None).
    ...
    def test_json_api_client_accepts_custom_timeout(self):
        ...
        self.assertEqual(client.timeout, 10,
                        f"Expected timeout=10, got {client.timeout}")
    ...
    def test_json_api_client_timeout_can_be_zero(self):
        ...
        self.assertEqual(client.timeout, 0,
                        f"Expected timeout=0, got {client.timeout}")
...
class TimeoutBehaviorTests(TestCase):
    ...
    def test_delivery_options_timeout_raises_exception(self, mock_get):
        ...
        self.assertIn("Delivery Options database is currently unavailable", 
